export const emailTemplates = {
  ...
  newMessage: ({ recipientName, senderName, messagePreview, truckInfo, conversationUrl }) => ({
    subject: `New message from ${senderName} - Tulsa Trucks`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0b4a8a;">You have a new message</h2>
        <p>Hi ${recipientName},</p>
        <p><strong>${senderName}</strong> sent you a message${truckInfo ? ` about <strong>${truckInfo}</strong>` : ''}:</p>
        <div style="background: #f8fafc; padding: 16px; border-radius: 8px; border-left: 4px solid #0b4a8a; margin: 16px 0;">
          <p style="margin: 0; color: #334155;">"${messagePreview}"</p>
        </div>
        <p><a href="${conversationUrl}" style="display: inline-block; background: #0b4a8a; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Message & Reply</a></p>
        <hr style="margin: 24px 0; border: none; border-top: 1px solid #e2e8f0;">
        <p style="color: #64748b; font-size: 14px;">Tulsa Trucks - team@tulsatrucksforsale.com</p>
      </div>
    `,
